...
encoding = {"F": "0", "B": "1", "R": "1", "L": "0"}


def read_boarding_passes(filepath):
    import os
    print(os.getcwd())
    boarding_passes = []
    with open(filepath) as fp:
        for line in fp:
            line = line.strip("\n")
            for letter, digit in encoding.items():
...
def part1():
    filepath = 'data/input_5.txt'
    boarding_passes = read_boarding_passes(filepath)
    seats = [get_seat(data) for data in boarding_passes]
    print("Max seat number: %d" % max(seats))
...
def part2():
    ...
    seats.sort()
    diffs = seats[1:] - seats[:(len(seats)-1)]
    ind = np.where(diffs == 2)
    print("My seat number: %d" % (seats[ind] + 1))
